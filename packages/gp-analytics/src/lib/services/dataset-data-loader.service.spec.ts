...
describe('GpDatasetDataLoaderService', () => {
  ...
  describe('parseJson', () => {
    ...
    it('throws error if specified dataPath is not found', () => {
      ...
      expect(() => service.parseJson(json, 'invalid.path')).toThrow(
        "Data path 'invalid.path' not found in payload",
      );
    });
  ...
  describe('mapRecordsToDatasetFields', () => {
    it('maps records using exact fieldName matches', () => {
      const rawRecords = [
        { customer_name: 'Wayne Enterprises', order_total: 99000, status: 'Shipped' },
      ];
  ...
  describe('loadData', () => {
    it('loads and maps data from raw JSON config', async () => {
      ...
      expect(result.matchedFields).toEqual(
        expect.arrayContaining(['customer_name', 'order_total', 'status']),
      );
    });

    it('loads and maps data from File config', async () => {
      const blob = new Blob([
        JSON.stringify([{ customer_name: 'File Corp', order_total: 100 }]),
    ...
    it('loads and maps data from API config using fetch', async () => {
      const mockApiResponse = [
        { customer_name: 'API Cloud', order_total: 3000, status: 'Completed' },
      ];
    ...
    it('handles API fetch errors gracefully', async () => {
      ...
      try {
        ...
        await expect(service.loadData(config, mockFields)).rejects.toThrow(
          'HTTP 404: Not Found',
        );
      } finally {
        globalThis.fetch = originalFetch;
