...
import {
  ...
  SchemaPreset,
  SchemaSourceConfig,
} from '../models/schema-data-source.model';
...
export class GpSchemaDataLoaderService {
  ...
  async loadSchema(config: SchemaSourceConfig): Promise<LoadedSchemaResult> {
    ...
    switch (config.type) {
      ...
      case 'file': {
        if (!config.file) throw new Error('No schema file selected');
      ...
      case 'api': {
        if (!config.url) throw new Error('No API URL provided');
      ...
      case 'json': {
        if (!config.rawJson) throw new Error('No JSON schema payload provided');
  ...
  normalizeSchemaPayload(
    payload: any,
    dataPath?: string,
  ): { groupings: Grouping[]; relationships?: Relationship[] } {
    let target = payload;
    ...
    if (Array.isArray(target)) {
      ...
      if (target.groupings && Array.isArray(target.groupings)) {
        ...
        throw new Error(
          'Schema JSON must contain an array of groups/groupings or tables',
        );
      }
    ...
    const normalizedGroupings: Grouping[] = rawGroupings.map((g, gIdx) =>
      this.normalizeGrouping(g, gIdx),
    );
  ...
  private normalizeTable(rawTable: any, parentGroupId: string, idx: number): Table {
    ...
    if (Array.isArray(rawTable.fields)) {
      if (rawTable.fields.length > 0 && Array.isArray(rawTable.fields[0].fields)) {
        ...
        fieldsList = rawTable.fields.map((fg: any, fgIdx: number) =>
          this.normalizeFieldGrouping(fg, tableId, fgIdx),
        );
      } else {
    ...
    if (Array.isArray(rawLookup) && rawLookup.length > 0) {
      lookupValues = rawLookup.map((item: any) => {
        if (typeof item === 'object' && item !== null && 'value' in item) {
          const displayValue = item.displayValue && typeof item.displayValue === 'object'
            ? item.displayValue
