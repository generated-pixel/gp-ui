...
- **Locked & Fixed Placement**:
  - `locked` / `lockedInPlace`: Anchored in place — never displaced by other widgets, and prevents widgets from being dropped over it.
  - `fixed` / `fixedInPlace`: Widget cannot be moved or resized by user dragging.
