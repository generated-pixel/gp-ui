...
export class GpCssGenerator {
  ...
  private resolveBaseRule(candidate: string): string | null {
    ...
    const displayMap: Record<string, string> = {
      ...
      // Transforms
      transform:
        'transform: translate(var(--gp-translate-x, 0), var(--gp-translate-y, 0)) rotate(var(--gp-rotate, 0)) scaleX(var(--gp-scale-x, 1)) scaleY(var(--gp-scale-y, 1));',
      'transform-gpu':
        'transform: translate3d(var(--gp-translate-x, 0), var(--gp-translate-y, 0), 0) rotate(var(--gp-rotate, 0)) scaleX(var(--gp-scale-x, 1)) scaleY(var(--gp-scale-y, 1));',
    ...
    if (fontWeightMap[candidate]) {
      return fontWeightMap[candidate];
    }
    ...
    if (candidate.startsWith('backdrop-blur')) {
      ...
      if (val) {
        return `backdrop-filter: blur(${val});`;
      }
    }
