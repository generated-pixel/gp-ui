...
- **Utility-First CSS Engine (@generatedpixel/gp-css)**: Modern JIT utility scanner & compiler featuring direct gp-theme design token integration, glassmorphism (glass), glow effects (glow), state modifiers (hover:, ocus:, dark:, responsive breakpoints), and @gp-css directives.
...
Include theme tokens and utility directives in your global styles.scss or CSS entrypoint:
...
|---|---|
| 
pm run build | Builds all monorepo packages (gp-css, gp-ui-theme, gp-ui-icons, gp-ui) and demo app |
| 
pm run start | Runs the interactive Demo & Documentation application locally |
| 
pm run lint | Validates architectural rules (Standalone, OnPush, isolation) |
| 
pm run test | Runs unit test suites (gp-css compiler tests & component spec verifications) |
| 
pm run generate component <name> | Scaffolds a new component, test spec, story, and styles |
| 
pm run release | Automates semantic version bump, changelog update, and packaging |
